...
impl<'a> Interpolator<'a> {
  ...
  pub fn resolve(&self, resolvable: &str) -> String {
    INTERPOLATION_REGEX
      .replace_all(resolvable, |caps: &Captures| {
        ...
        if let Some(item) =
          self.resolve_global_reference(capture)
        {
          return item;
        }

        if let Some(item) =
          self.resolve_env_file_reference(capture)
        {
          return item;
        }

        if let Some(item) =
          self.resolve_environment_interpolation(capture)
        {
          return item;
        }

        if let Some(item) =
          self.resolve_context_interpolation(capture)
        {
          return item;
        }

        panic!(
          "{} Couldn't resolve variable '{}'",
          "ERROR:".yellow().bold(),
          capture
        );
      })
  ...
  fn resolve_env_file_reference(
    &self,
    value: &str,
  ) -> Option<String> {
    self.context.get(value)?.as_str().map(|s| s.to_owned())
  }

  fn resolve_environment_interpolation(
    &self,
    value: &str,
  ) -> Option<String> {
  ...
  fn resolve_context_interpolation(
    &self,
    value: &str,
  ) -> Option<String> {
    ...
    if let Some(item) =
      json!(self.context).pointer(&val).to_owned()
    {
...
mod tests {
  ...
  fn interpolates_variables() {
    ...
    assert_eq!(
      interpolated,
      "http://example.com/users/12/view/12/chunked"
    );
  }
  ...
  fn interpolates_variables_nested() {
    ...
    assert_eq!(
      interpolator.resolve("{{ Null }}"),
      "".to_string()
    );
    assert_eq!(
      interpolator.resolve("{{ Bool }}"),
      "true".to_string()
    );
    assert_eq!(
      interpolator.resolve("{{ Number }}"),
      "12".to_string()
    );
    assert_eq!(
      interpolator.resolve("{{ String }}"),
      "string".to_string()
    );
    assert_eq!(
      interpolator.resolve("{{ Array }}"),
      "[\"a\",\"b\",\"c\"]".to_string()
    );
    assert_eq!(
      interpolator.resolve("{{ Object }}"),
      "{\"this\":\"that\"}".to_string()
    );
    assert_eq!(
      interpolator.resolve(
        "{{ Nested.this.that.those[2].deee.eeee }}"
      ),
      "eeep".to_string()
    );
    assert_eq!(
      interpolator
        .resolve("{{ ArrayNested[0].a[1].aaa[0].aaaa }}"),
      "123".to_string()
    );
    assert_eq!(
      interpolator
  ...
  fn interpolates_numnamed_variables() {
    ...
    assert_eq!(
      interpolated,
      "http://example.com/postalcode/90210/view/90210"
    );
  }
  ...
  fn interpolates_environment_variables() {
    ...
    let interpolator = Interpolator::new(&context);
    let url = String::from(
      "http://example.com/postalcode/{{ FOO }}",
    );
    let interpolated = interpolator.resolve(&url);

    assert_eq!(
      interpolated,
      "http://example.com/postalcode/BAR"
    );
  }
}
