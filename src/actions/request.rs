...
impl Pick {
  pub fn new(value: i64, with_items: &[Yaml]) -> Self {
    if value.is_negative() {
      panic!(
        "pick option should not be negative, but was {}",
        value
      );
...
impl Request {
  pub fn new(
    ...
    let mut with_keys =
      item.as_hash().unwrap().keys().filter(|key| {
        key.as_str().unwrap().contains("with")
      });
    if with_keys.clone().count() > 1 {
      panic!(
        "{} 'with' attributes are mutually exclusive",
        "ERROR".yellow().bold()
      );
    }
    ...
    let shuffle =
      item["shuffle"].as_bool().unwrap_or(false);
    let pick = Pick::new(
      item["pick"]
        .as_i64()
        .unwrap_or(with_items.len() as i64),
      &with_items,
    );
    let base = extract_optional(item, "base");
    let url = extract(item, "url");

    let method = if let Some(v) =
      extract_optional(&item["request"], "method")
    {
      v.to_uppercase()
    } else {
      "GET".to_string()
  ...
  async fn send_request(
    ...
    if let Some(val) = with_item {
      ...
      for (key, val) in map {
        context.insert(
          ...
          Value::String(val.clone().into_string().unwrap()),
        );
      }
    }
    ...
    let interpolated_base_url = if let Some(base_url) =
      self.base.clone()
    {
      match context.get("urls") {
        Some(value) => {
          if let Some(url_map) = value.as_object() {
            ...
            interpolator
              .resolve(joined_url.to_str().unwrap())
          } else {
            panic!(
    ...
    let url = Url::parse(&interpolated_base_url)
      .expect("Invalid url");
    let domain = format!(
      "{}://{}:{}",
      url.scheme(),
    ...
    let (client, request) = {
      ...
      let client =
        pool2.entry(domain).or_insert_with(|| {
    ...
    for (key, val) in self.headers.iter() {
      ...
      headers.insert(
        ...
        HeaderValue::from_str(&interpolated_header)
          .unwrap(),
      );
    }
  ...
  async fn execute_one_request(
    ...
    let (res, duration_ms) = self
      .send_request(context, pool, config, with_item)
      .await;
...
impl Runnable for Request {
  async fn execute(
    ...
    if self.with_items.is_empty() {
      self
        .execute_one_request(
          context, pool, config, reports, None,
        )
        .await;
    } else {
      ...
      for with_item in
        with_items.iter().take(self.pick.inner())
      {
        self
          .execute_one_request(
            context,
...
fn csv_values(parent_path: &str, item: &Yaml) -> Vec<Yaml> {
  let (with_items_path, quote_char) =
    if let Some(with_items_path) =
      item["with_items_from_csv"].as_str()
    {
      (with_items_path, b'\"')
    } else if let Some(_with_items_hash) =
      item["with_items_from_csv"].as_hash()
    {
      let with_items_path = item["with_items_from_csv"]
        ["file_name"]
        .as_str()
        .expect("Expected a file_name");
      let quote_char = item["with_items_from_csv"]
        ["quote_char"]
        .as_str()
        .unwrap_or("\"")
        .bytes()
  ...
  let with_items_filepath =
    Path::new(parent_path).with_file_name(with_items_path);
...
fn file_values(
  ...
  let with_items_path = if let Some(with_items_path) =
    item["with_items_from_file"].as_str()
  {
    with_items_path
  } else {
    unreachable!();
...
fn iter_values(item: &Yaml) -> Vec<Yaml> {
  if let Some(with_iter_items) =
    item["with_items_range"].as_hash()
  {
    ...
    let vstart: &Yaml = with_iter_items
      .get(&lstart)
      .expect("Start property is mandatory");
    let vstep: &Yaml =
      with_iter_items.get(&lstep).unwrap_or(&init);
    let vstop: &Yaml = with_iter_items
      .get(&lstop)
      .expect("Stop property is mandatory");

    let start: i64 =
      vstart.as_i64().expect("Start needs to be a number");
    let step: i64 =
      vstep.as_i64().expect("Step needs to be a number");
    let stop: i64 =
      vstop.as_i64().expect("Stop needs to be a number");
...
fn log_request(request: &reqwest::Request) {
  ...
  write!(message, " {} {},", "URL:".bold(), request.url())
    .unwrap();
  write!(
    message,
    " {} {},",
